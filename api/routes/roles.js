...
router.post('/add', async (req, res) => {
    ...
    try {
        ...
        if (!body.permissions || !Array.isArray(body.permissions) || body.permissions.length == 0) {
            throw new CustomError(Enum.HTTP_CODES.BAD_REQUEST, "Validation Error", "permission Kısmı Dolu Olmalı")
        }
        let role = new Roles(
            {
                role_name: body.role_name,
                is_active: true,
                created_by: req.user?.id//req user id var ise id yi al fakat yoksa null olabileceğinden hata vermez 
            }
        )
        await role.save();
...
router.post('/update', async (req, res) => {
    ...
    try {
        ...
        if (!body._id) throw new CustomError(Enum.HTTP_CODES.BAD_REQUEST, "Validation Error", "Id Kısmı Dolu Olmalı");
